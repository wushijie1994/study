...
MapCharts.prototype = {
    ...
    * */
    render(){
        const isChinaDataReady=this.chinaData===undefined?false:true;
        const isPathDataReady=this.pathData===undefined?false:true;
        /*如果地图数据和路径数据都存在那么可以开始绘制*/
        if(isChinaDataReady&&isPathDataReady){
            //    先配置缩放比
            this.setMatrix();
            //渲染所有的路径数据
            let covertObj=this.renderAllPath();
            /*将计算地图得出的数据合并到数据源上面*/
            this.fixedChinaData(covertObj);
            /*根据数据渲染地图*/
            this.renderMapData();
        }
    },
    /*
    * @return object（covertObj）：返回地图路径的范围数据集
    * */
    renderAllPath(){
        //清空对应的元素
        this.clearPath();
        let covertObj = {};
        for (let areaKey in this.pathData) {
            let covertObjItem = this.convertToXYList(this.pathData[areaKey]);
            /*绘制路径*/
            this.renderPath(covertObjItem.latLng, areaKey);
            //往对应的区域数据中里面去copy转换后的对象
            delete covertObjItem.latLng;
            covertObj[areaKey] = covertObjItem;
        }
        /*地图绘制完成就不需要路径数据了，释放内存*/
        delete this.pathData;
        return covertObj;
    },
    /*对省市级单位坐标数据进行修正*/
    fixedChinaData: function (covertObj) {
        var data = null;
        //全国
        if (this.level === 1) {
            data = this.chinaData;
        } else {  //省份
            data = this.getCoordinateByMapId(this.provinceKey).list;
        }

        for (var key in data) {
            $.extend(data[key], covertObj[key]);
        }
    },
    /*
     * 设置地图的缩放比例，这个应该是在绘制地图区域之前执行
     * */
    setMatrix: function () {
        var _this = this;
        var areaData = null, data = null, key = _this.provinceKey;
        //全国
        if (_this.level === 1) {
            areaData = _this.chinaData;
            data = _this.chinaData;
        } else {  //省份
            areaData = _this.getCoordinateByMapId(key);
            data = areaData.list;
        }
    ...
    * */
     ...
     * 创建对应的文本
     * */
    renderText: function (name, textPoint, mapId) {
        var _this = this;
    ...
    * mapData:地图数据
    * */
    renderCircle: function (point, mapId) {
        let _this = this;
        /*计算变换后的坐标*/
     ...
     * 绘制区域
     * */
    renderPath: function (latLng, mapId) {
        var _this = this;
